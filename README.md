# ⚾ TossBaseball: KBO 승부예측 백엔드 엔진 (Step 2)

**TossBaseball**은 KBO 리그 경기 결과 예측 데이터를 체계적으로 관리하고 집계하기 위한 고성능 백엔드 시스템입니다. 본 프로젝트는 **'앱인토스(App-in-Toss)'** 플랫폼 정식 등재를 목표로 하며, 대규모 승부예측 데이터를 안정적으로 처리할 수 있는 아키텍처를 지향합니다.
